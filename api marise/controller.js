...
router.post('/sendData', (request, response) => {
    ...
    luminosidade = (ArduinoDataLuminosity.List[ArduinoDataLuminosity.List.length - 1]) * 10;
   
    var fkid = parseInt(Math.random()*100+1);

    var sql = `INSERT INTO medidas (fkid, luminosidade, dataLog) VALUES ('${fkid}', ?,'${agora()}')`;

    db.query(sql, luminosidade, function(err, result) {
        if (err) throw err;
        console.log("Number of records inserted: " + result.affectedRows);
        console.log(ArduinoDataLuminosity.ListHour.reduce((a, b) => a + b, 0))
      });
